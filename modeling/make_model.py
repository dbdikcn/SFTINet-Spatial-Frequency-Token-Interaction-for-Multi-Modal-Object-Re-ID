...
from modeling.fusion_part.FTM import Frequency
from modeling.fusion_part.SFM import SFM
...
class TOPReID(nn.Module):
    def __init__(self, num_classes, cfg, camera_num, view_num, factory):
        ...
        self.FTM = Frequency(self.mix_dim // 6, self.num_head // 3)
        self.SFM = SFM(2304)#768 2304
        # self.FTM = Frequency_Transformer(self.mix_dim // 6, self.num_head // 3)
    def load_param(self, trained_path):
        param_dict = torch.load(trained_path)
        for i in param_dict:
            self.state_dict()[i.replace('module.', '')].copy_(param_dict[i])
    ...
    def forward(self, x, cam_label=None, label=None, view_label=None):
        if self.training:
            RGB = x['RGB']
            NI = x['NI']#torch.Size([64, 3, 256, 128])
            TI = x['TI']
            Fre_feature, mask_token, loss = self.FTM(RGB, NI, TI)
            NI_cash, NI_score, NI_global = self.NI(NI, cam_label=cam_label, view_label=view_label)
            TI_cash, TI_score, TI_global = self.TI(TI, cam_label=cam_label, view_label=view_label)
            RGB_cash, RGB_score, RGB_global = self.RGB(RGB, cam_label=cam_label, view_label=view_label)
            #torch.Size([64, 129, 768])原代码为list global=torch.Size([64, 768])score=torch.Size([64, 171])

            ori = torch.cat([RGB_global, NI_global, TI_global], dim=-1)#torch.Size([64, 2304])
            ori_global = self.bottleneck_ViT(ori)#torch.Size([64, 2304])
            ori_score = self.classifier_ViT(ori_global)#score=torch.Size([64, 171])
            TPM_feature = self.TPM(RGB_cash[self.layer], NI_cash[self.layer], TI_cash[self.layer], mask_token)#torch.Size([64, 2304])
            TPM_feature = self.SFM(Fre_feature, TPM_feature)
            if self.re:
            ...
            if self.re:
                if self.direct:
                    return TPM_score, TPM_feature, ori_score, ori, loss_re + loss
                else:
                    return TPM_score, TPM_feature, RGB_score, RGB_global, NI_score, NI_global, TI_score, TI_global, loss_re + loss
            else:
                if self.direct:
            ...
            Fre_feature, mask_token = self.FTM(RGB, NI, TI)

            NI_cash, NI_global = self.NI(NI, cam_label=cam_label, view_label=view_label)
            TI_cash, TI_global = self.TI(TI, cam_label=cam_label, view_label=view_label)
            RGB_cash, RGB_global = self.RGB(RGB, cam_label=cam_label, view_label=view_label)
            TPM_feature = self.TPM(RGB_cash[self.layer], NI_cash[self.layer], TI_cash[self.layer], mask_token)
