...
class Attention(nn.Module):
    ...
    def forward(self, x, get_attn=False):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)#torch.Size([64, 12, 129, 64])
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)#torch.Size([64, 12, 129, 64])

        attn = (q @ k.transpose(-2, -1)) * self.scale#torch.Size([64, 12, 129, 129])
        attn = attn.softmax(dim=-1)#torch.Size([64, 12, 129, 129])
        attn = self.attn_drop(attn)#torch.Size([64, 12, 129, 129])
        # if get_attn:
        #     return attn
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)#torch.Size([64, 129, 768])
        x = self.proj(x)#torch.Size([64, 129, 768])
        x = self.proj_drop(x)#torch.Size([64, 129, 768])
        return x

    
class Block(nn.Module):#dim = 768
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.attn = HybridTokenMixer(dim, num_heads)
    ...
    def forward(self, x, get_att=False):
        x = x + self.drop_path(self.attn(self.norm1(x)))#torch.Size([64, 129, 768])
        x = x + self.drop_path(self.mlp(self.norm2(x)))#torch.Size([64, 129, 768])
        return x#torch.Size([64, 129, 768])
...
class PatchEmbed_overlap(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape#torch.Size([64, 3, 256, 128])

        # FIXME look at relaxing size constraints
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x)#torch.Size([64, 768, 16, 8])
...
class Trans(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        B = x.shape[0]
        x = self.patch_embed(x)#torch.Size([64, 128, 768])b,hw,c

        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks  torch.Size([64, 1, 768])
        x = torch.cat((cls_tokens, x), dim=1)#torch.Size([64, 129, 768])
        ...
        x = self.pos_drop(x)#torch.Size([64, 129, 768])
        cash_x = []
